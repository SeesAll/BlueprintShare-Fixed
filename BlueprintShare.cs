...
{
    [Info("Blueprint Share", "c_creep, modified by SeesAll", "1.4.6")]
    ...
    class BlueprintShare : RustPlugin
    {
        ...
        private void OnTechTreeNodeUnlocked(Workbench workbench, TechTreeData.NodeInstance node, BasePlayer player, PooledList<ItemDefinition> items)
        {
            if (!config.TechTreeSharingEnabled || player == null || items == null) return;

            foreach (var item in items)
            {
                if (item == null) continue;

                DebugLog($"{player.displayName} Started TechTree Unlock For {item.displayName.translated}");
