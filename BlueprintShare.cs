...
{
    [Info("Blueprint Share", "c_creep, modified by SeesAll", "1.4.5")]
    ...
    class BlueprintShare : RustPlugin
    {
        ...
        private Timer pendingDataSaveTimer;

        private const float dataSaveDebounceDelay = 30f; // Seconds
        ...
        private void OnTechTreeNodeUnlocked(Workbench workbench, ItemDefinition itemDefinition, BasePlayer player)
        {
            if (!config.TechTreeSharingEnabled || workbench == null || itemDefinition == null || player == null) return;

            DebugLog($"{player.displayName} Started TechTree Unlock For {itemDefinition.displayName.translated}");

            TryShareBlueprint(itemDefinition, player);
        }
        ...
        private void OnTeamLeave(RelationshipManager.PlayerTeam team, BasePlayer player)
        {
            ...
            ClearShareTargetCache(player.userID);
        ...
        private void OnTeamKick(RelationshipManager.PlayerTeam team, BasePlayer player, ulong targetId)
        {
            ...
            ClearShareTargetCaches(player.userID, targetId);
        ...
        private void OnFriendAdded(string playerIdStr, string friendIdStr)
        {
            ...
            ClearShareTargetCaches(playerId, friendId);
        ...
        private void OnClanMemberJoined(ulong playerId, string clanName)
        {
            ...
            ClearShareTargetCache(playerId);
        ...
        private void OnClanDisbanded(List<string> memberIds)
        {
            ...
            {
                var sharedBlueprints = GetSharedBlueprints(memberId, ShareType.Clans);
                ...
                {
                    RemoveBlueprints(playerId, sharedBlueprints, ShareType.Clans);
                    processedPlayers++;
                }
                else
                {
                    // Fallback: if we somehow can't parse the player id, at least clear our tracking data
                    RemoveBlueprintsFromDatabase(ShareType.Clans, memberId, string.Empty);
                }
            }

            DebugLog($"Removed clan blueprints for {processedPlayers} player(s) on clan disband");
        }
        ...
        private void ShareBlueprint(BasePlayer sharer, List<ulong> targetIds, ItemDefinition item)
        {
            ...
            foreach (var targetId in targetIds)
            {
                ...
                var additionalUnlocks = item.Blueprint?.additionalUnlocks;
                if (additionalUnlocks != null)
                {
                    foreach (var blueprint in additionalUnlocks)
                    {
        ...
        private bool SomeoneWillLearnBlueprint(ulong playerId, ItemDefinition item)
        {
            ...
            foreach (var targetId in targetIds)
            {
                ...
                foreach (var blueprintItem in additionalUnlocks)
                {
        ...
        private void ClearShareTargetCache(ulong playerId)
        {
            if (shareTargetCache == null) return;

            shareTargetCache.Remove(playerId);

            if (shareTargetCacheTimers.TryGetValue(playerId, out var existingTimer))
            {
        ...
        private void ClearShareTargetCaches(params ulong[] playerIds)
        {
        ...
        private void ScheduleDataSave()
        {
            ...
            if (pendingDataSaveTimer != null && !pendingDataSaveTimer.Destroyed)
            {
                pendingDataSaveTimer.Destroy();
            }

            pendingDataSaveTimer = timer.Once(dataSaveDebounceDelay, () =>
            {
        ...
        private void CancelScheduledDataSave()
        {
            if (pendingDataSaveTimer == null) return;

            if (!pendingDataSaveTimer.Destroyed)
            {
                pendingDataSaveTimer.Destroy();
            }
